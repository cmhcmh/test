...
class AreasubAction extends CommonAction {
    ...
    public function index() {
		...
		$where = ' AND level=2 AND a_parentid='.$filter['p_id'];//地区等级
	...
	public function insert(){
		...
		if($insertId){
			...
			$this->success('添加成功！！',U('Areasub/add/',array('prefix'=>$prefix,'p_id'=>$data['a_parentid'])));
		}else{
			$this->error('添加失败！！',U('Area/add/',array('prefix'=>$prefix,'p_id'=>$data['a_parentid'])));
		}
		exit();
	}
	/**
	...
	public function update(){
		...
		if($updateId){
			...
			$this->success('修改成功！！',U('Areasub/index/',array('prefix'=>$prefix,'p_id'=>$data['a_parentid'])));
		}else{
			$this->success('修改失败！！',U('Areasub/index/',array('prefix'=>$prefix,'p_id'=>$data['a_parentid'])));
		}
		exit();
	}
	/**
