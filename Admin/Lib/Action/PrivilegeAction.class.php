...
class PrivilegeAction extends CommonAction {
    ...
    public function index() {	
        ...
        $filter['c_name']    = empty($_REQUEST['c_name']) ? '' : trim($_REQUEST['c_name']);
        $filter['company_id']    = empty($_REQUEST['company_id']) ? '' : intval($_REQUEST['company_id']);

        $filter['record_count'] = $count = D("Privilege")->listPrivilegeCount($filter);
        import("ORG.Util.Page");       //载入分页类
        $page = new Page($count, 20);
        $showPage = $page->show();
        $this->assign('page',        $showPage);
        $filter['firstRow'] = $page->firstRow;
        $filter['listRows'] = $page->listRows;
        
		$this->assign('admin_list',  D('Privilege')->get_admin_userlist($filter));
        $this->display();
    }
    ...
    public function edit() {
		...
		$user_info = $M_admin_user->where(array('user_id'=>$id))->field('user_id, user_name, email, password, agency_id, role_id,company_id')->find();

		if(!empty($user_info['company_id']))
		{

            $company = M('company')->where('c_id='.$user_info['company_id'])->find();
            $user_info['province_id']=$company['c_province'];
            $user_info['city_id']=$company['c_city'];
		}
					//城市
		if($user_info['province_id']){

			$this->city_list=$this->citysublist($user_info['province_id'],2);
		}
		//地区
		if($user_info['city_id']){
			$this->district_list=$this->citysublist($user_info['city_id'],3);

		}

		//权限公司

		$this->company_list=$this->getcompanylist($user_info['city_id']);
	...
	function update()
	{
       ...
       // print_R($row['action_list']);exit;
		...
		if($row['action_list']!='all')
		{
           ...
           {
				$data['company_id']     = $_POST['company_id'];
				$datacompany_str ="company_id=".$_POST['company_id'];
           }
		}
		//更新管理员信息
	   ...
	   if($this->admin_info!="all")
	   {
	   	$this->success('修改管理员信息成功！',U('Privilege/edit',array('id'=>$this->admin_info['user_id'])));
	   }else
	   {
