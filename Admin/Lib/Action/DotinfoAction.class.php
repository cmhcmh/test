...
class DotinfoAction extends CommonAction {
  ...
  /**
    ...
    if($this->admin_info['action_list']=='all'){
          ...
          $company_id   = !empty($_REQUEST['c_id'])?$_REQUEST['c_id']:0;
          if($company_id)
          {
            $companyinfo=M()->table("tp_company")->where('c_id='.$company_id)->find();;
    ...
    $this->assign("company_id",$company_id);
        $this->assign("new_company_id",$company_id);
  ...
  public function page(){
    ...
    if($this->admin_info['action_list']=='all'){
          ...
          //$data['admin_city_id']    = $this->_post("admin_city_id","","");
         // $company_id   = 0;
       $company_id   = !empty($_POST['new_company_id'])?$_POST['new_company_id']:0;
    }else
    {
          $company_id = $this->admin_info['company_id'];
              //$companyinfo = $this->getcompanyinfo($this->admin_info['company_id']);//获取用户城市的id
         ...
         if($info)
    {
      ...
      $this->success('更新成功！！',U('Dotinfo/index/',array('w_companyid'=>$data['w_companyid'],'prefix'=>$prefix,'c_id'=>$company_id)));
      }else{
        $this->error('更新失败！！',U('Dotinfo/index/',array('w_companyid'=>$data['w_companyid'],'prefix'=>$prefix,'c_id'=>$company_id)));
      } 
    ...
    }else
    {
      ...
      if($insertId){
        ...
        $this->success('添加成功！！',U('Dotinfo/index/',array('w_companyid'=>$company_id,'prefix'=>$prefix,'c_id'=>$company_id)));
      }else{
        $this->error('添加失败！！',U('Dotinfo/index/',array('w_companyid'=>$company_id,'prefix'=>$prefix,'c_id'=>$company_id)));
      }
