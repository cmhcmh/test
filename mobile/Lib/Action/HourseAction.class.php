...
class HourseAction extends CommonAction {
    ...
    public function index() {
        ...
        /************************轮播图*****************************/
        $this->banner=M('ads')->where("cat_id=3 and title='热装楼盘'")->getField('original_img');
        //分类星系
        ...
        //上层分类
        $cat_info=M('articlecat')->where('cat_id=19')->find();
        ...
        $this->hourse      = M('article')->where($this->com_where.' and cat_id=19'.$where)->join('tp_area on city_id=a_id')->order($this->company_sort)->limit(5)->select();
    ...
    public function load(){
        ...
        $arr    = M('article')->where($this->com_where.' and cat_id=19'.$where)->join('tp_area on city_id=a_id')->order($this->company_sort)->limit($pages,5)->select();
