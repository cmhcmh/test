<?php
	 ...
	 * @return  mix
	 */
	function article_cat_list($prefix='tp_', $cat_id = 0, $selected = 0, $re_type = true, $level = 0)
	{
		static $res = NULL;
		...
		if ($res === NULL)
		{
			$sql = "SELECT c.*, COUNT(s.cat_id) AS has_children, COUNT(a.article_id) AS aricle_num ".
			   " FROM " . $prefix . "articlecat AS c".
			   " LEFT JOIN " . $prefix . "articlecat AS s ON s.parent_id=c.cat_id".
			   " LEFT JOIN " . $prefix . "article AS a ON a.cat_id=c.cat_id".
			   " GROUP BY c.cat_id ".
			   " ORDER BY parent_id, sort_order ASC";
			$model = M()->table($prefix."articlecat");
			$res = $model->query($sql);
		}
		if (empty($res) == true)
		{
	 ...
	 * @return  array
	 */
	function get_article_categories_tree($cat_id = 0,$prefix)
	{
		$articleCat=M()->table($prefix.'articlecat');
		if ($cat_id > 0)
		{
			$parentCat=$articleCat->where(array('cat_id'=>$cat_id))->field('parent_id')->find(); 
			$parent_id = $parentCat['parent_id'];
		...
		*/
		...
		if ($sameGradeCount || $parent_id == 0)
		{
			...
			foreach ($res AS $row)
			{
				...
				{
					$cat_arr[$row['cat_id']]['cat_id'] = get_article_child_tree($row['cat_id'],$prefix);
				}
	...
	function get_article_child_tree($tree_id = 0,$prefix)
	{
		$articleCat=M()->table($prefix.'articlecat');
		$three_arr = array();
		...
		if ($sameGradeCount || $tree_id == 0)
		{
			...
			foreach ($res AS $row)
			{
			   ...
			   if (isset($row['cat_id']) != NULL)
				{
					   $three_arr[$row['cat_id']]['cat_id'] = get_article_child_tree($row['cat_id'],$prefix);
				}
			}
		}
		return $three_arr;
	 ...
	 * @return void
	 */
	function get_article_children ($prefix,$cat = 0)
	{
		return db_create_in(array_unique(array_merge(array($cat), array_keys(article_cat_list($prefix,$cat, 0, false)))), 'cat_id');
	}
	 ...
	 * @return  mix
	 */
	function goods_cat_list($prefix='tp_',$cat_id = 0, $selected = 0, $re_type = true, $level = 0)
	{
		static $res = NULL;
		...
		if ($res === NULL)
		{
			$sql = "SELECT c.*, COUNT(s.cat_id) AS has_children, COUNT(a.goods_id) AS aricle_num ".
			   " FROM " . $prefix . "goodscat AS c".
			   " LEFT JOIN " . $prefix . "goodscat AS s ON s.parent_id=c.cat_id".
			   " LEFT JOIN " . $prefix . "goods AS a ON a.cat_id=c.cat_id".
			   " GROUP BY c.cat_id ".
			   " ORDER BY parent_id, sort_order ASC";

			$model = M()->table($$prefix."goodscat");
			$res = $model->query($sql);
		}
		if (empty($res) == true)
		{
	...
	function goods_cat_list2($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
	{
		static $res = NULL;
	 ...
	 * @return  array
	 */
	function get_goods_categories_tree($prefix='tp_',$cat_id = 0)
	{
		$goodsCat=M()->table($prefix.'goodscat');
		if ($cat_id > 0)
		{
			$parentCat=$goodsCat->where(array('cat_id'=>$cat_id))->field('parent_id')->find(); 
			$parent_id = $parentCat['parent_id'];
		...
		*/
		...
		if ($sameGradeCount || $parent_id == 0)
		{
			...
			foreach ($res AS $row)
			{
				...
				if (isset($row['cat_id']) != NULL)
				{
					$cat_arr[$row['cat_id']]['cat_id'] = get_goods_child_tree($prefix,$row['cat_id']);
				}
	...
	function get_goods_child_tree($prefix='tp_',$tree_id = 0)
	{
		$goodsCat=M()->table($prefix.'goodscat');
		$three_arr = array();
		...
		if ($sameGradeCount || $tree_id == 0)
		{
			...
			foreach ($res AS $row)
			{
			   ...
			   if (isset($row['cat_id']) != NULL)
				{
					   $three_arr[$row['cat_id']]['cat_id'] = get_goods_child_tree($prefix,$row['cat_id']);
				}
			}
		}
		return $three_arr;
	 ...
	 * @return void
	 */
	function get_goods_children ($prefix='tp_',$cat = 0)
	{
		return db_create_in(array_unique(array_merge(array($cat), array_keys(goods_cat_list($prefix,$cat, 0, false)))), 'cat_id');
	}
