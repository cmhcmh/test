    ...
    public function index() {
     ...
     public  function message(){
      ...
      if(eregi("[^\x80-\xff]",$data['hourse'])){ 
        ...
        $data['mianji']=addslashes(htmlspecialchars($_POST['mianji']));
		...
		session_start();
		$now = time(); //当前时间
		if (isset($_SESSION['last_submit']) && $now - $_SESSION['last_submit'] < 120) {
			//上次提交时间距现在超过60s
		...
		*/
        ...
        if($add){
			...
			$areaname2 = M('area')->where('a_id='.$data['city_id'])->find();
				$weburl1 = 'www'.$_SERVER['HTTP_HOST'];
				if(stripos($weburl1,'hxzs1998.cn',TRUE)){
					$host11 = '九八网站';
				}elseif(stripos($weburl1,'hxdec.com.cn',TRUE)){
					$host11 = '点COM';
				}
				if(empty($com_id['c_name'])){
					$com_id['c_name'] =  "$host11";
				}
