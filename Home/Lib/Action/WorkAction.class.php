...
class WorkAction extends CommonAction {
    ...
    public function index() {
       ...
       // $this->banner=M('ads')->where("cat_id=3 and title='案例展示'")->getField('original_img');
        ...
        $keywords=!empty($_REQUEST['work_id'])?addslashes(htmlspecialchars($_REQUEST['work_id'])):'';
        $keywords1=!empty($_REQUEST['keywords1'])?addslashes(htmlspecialchars($_REQUEST['keywords1'])):'';
        ...
        $this->site_keywords    = $new_cat_info['work_id']? $new_cat_info['work_id']:$this->site_info['work_id'];
        $this->site_description = $new_cat_info['description']? $new_cat_info['description']:$this->site_info['description'];
        ...
        if($keywords){
                 ...
                 $where.=" and a.construct_data like '%$keywords%'";
        }
		if($keywords1){

                 $where.=" and ( a.title like '%$keywords1%')";
        ...
        $count      = M('article')->alias('a')->where($this->com_where.' and cat_id=20 '.$where)->count();// 
        ...
        $this->worklist      = M('article')->alias('a')->field('a.*,b.article_id as design_id,b.title as design_title,c.article_id as hourse_id,c.title as hourse_title')->join('tp_article  as  b  on b.article_id = a.be_design')->join('tp_article  as  c  on c.article_id = a.be_hourse')->where($this->com_table_where.' and a.cat_id=20'.$where)->order('a.sort_order asc,a.add_time desc')->limit($Page->firstRow.','.$Page->listRows)->select();//
        $worklist = $this->worklist;
        foreach($worklist as $key=>$val)
        {

            $worklist[$key]['construct_data'] = unserialize($val['construct_data']);
        }
    ...
    public function detail(){
        ...
        $article_detail['construct_data'] = unserialize($article_detail['construct_data']);
        ...
        $this->hourse_info = M('article')->where("article_id=".$article_detail['be_hourse'])->find();
         ...
         //设计师
        $this->design_info = M('article')->where("article_id=".$article_detail['be_design'])->find();
